...
- Resolve the diff scope by running the scope resolver before doing any review analysis.

**Script calls:**
- `bash "$SCOPE" --mode workspace`
- `bash "$SCOPE" --mode branch --base main`
- `bash "$SCOPE" --mode range --from abc123 --to def456`

**How to use the script output:**
- Read `=== CODE_REVIEW_SUMMARY ===` first to understand the selected scope.
- Treat `=== CODE_REVIEW_DIFF ===` as the source of truth for the review.
...
- **Large diff (>500 lines)**: Summarize by file first, then review in batches by module or feature area.
...
| File | Purpose |
|------|---------|
| `scripts/resolve-review-scope.sh` | Resolve the selected review scope and output summary + raw diff |
