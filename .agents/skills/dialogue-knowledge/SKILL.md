...
根据对话内容识别类型，按对应结构生成笔记：

| 类型 | 典型场景 | 笔记结构模板 |
|------|----------|-------------|
| `debug` | 排查错误、修复 bug | **问题现象** → **排查过程**（尝试了什么、排除了什么） → **根因** → **解决方案** → **教训** |
| `research` | 技术选型、方案调研 | **背景/目标** → **候选方案**（各自优劣） → **对比结论** → **最终选择及理由** |
| `implementation` | 实现功能 | **需求概述** → **技术方案** → **关键实现细节**（代码片段） → **踩坑与注意事项** |
| `optimization` | 性能优化、重构 | **问题指标** → **分析过程** → **优化手段** → **效果对比** |
| `learning` | 学习新概念 | **核心概念** → **关键要点** → **实践建议** → **延伸阅读** |

**生成笔记的要求：**
- 标题要具体，体现问题和解决方案（好：`FastAPI 路由中嵌套代码块导致 JSON 解析截断`，差：`修复了一个 bug`）
- 只提炼有价值的信息，不要复述对话过程
- 代码片段只保留关键的，不要大段搬运
- 一条对话生成一篇笔记，不要拆成多篇
- 参考 `references/example-notes/` 下的示例笔记了解格式和质量标准
