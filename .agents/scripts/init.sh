...
TOOL_DIRS_STR=$(detect_tool_dirs "$PROJECT_ROOT")

if [[ -z "$TOOL_DIRS_STR" ]]; then
    log_info "未检测到 AI 工具目录，无需链接"
    exit 0
fi
...
if $CLEAN_ONLY; then
    ...
    for tool_dir in "${_tool_dirs[@]}"; do
        ...
        for asset_type in "${ASSET_TYPES[@]}"; do
            clean_stale_links "$asset_type" "$tool_dir" "$PROJECT_ROOT"
        done
    done
    ...
    for asset_type in "${ASSET_TYPES[@]}"; do
        init_links "$asset_type" "$PROJECT_ROOT" "$AGENTS_DIR" "$TOOL_DIRS_STR"
    done
fi
